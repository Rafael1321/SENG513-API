...
module.exports.findMatch = async (req, res) => {
    ...
    try{
        ...
        if(!onlineUserIds.length) return res.type('json').status(404).send('Could not find a match. Try again later!');
        
        // Find user id's that have been matched with userId
        const matchedUserIds = [];
        const cursor = matching.find({ $or:[ {'firstUser':  userId}, {'secondUser': userId} ]}).cursor();
        ...
        if(!userIdsIntersect.length) return res.type('json').status(404).send('Could not find a match. Try again later!'); 

        // Retrieve each user from database and apply filters
        let searchedUser = null;
        let userFound = false;
        for(let i = 0; i < userIdsIntersect.length; i++){
            
            let userId = userIdsIntersect[i];
